...
func TestSessionModifySlotBackedDeclaredTemplateSetUnsetDefaultRequiredAndDuplicateCollision(t *testing.T) {
	...
	if internal := mustWorkingFactByID(t, session, first.Fact.ID()); internal.duplicateIndexForRevision(session.revision, session.factStore.compactSlotStore).kind != duplicateIndexSingleScalar {
		t.Fatalf("slot-backed duplicate index kind = %v, want %v", internal.duplicateIndexForRevision(session.revision, session.factStore.compactSlotStore).kind, duplicateIndexSingleScalar)
	}
	...
	if internal := mustWorkingFactByID(t, session, first.Fact.ID()); internal.duplicateIndexForRevision(session.revision, session.factStore.compactSlotStore).kind != duplicateIndexSingleScalar {
		t.Fatalf("slot-backed duplicate index kind after modify = %v, want %v", internal.duplicateIndexForRevision(session.revision, session.factStore.compactSlotStore).kind, duplicateIndexSingleScalar)
	}

...
type betaTokenRow struct {
	token tokenRef
	// outputHead anchors the row's joinOutputTable chain on positive join
	// nodes; it stays zero in filter and negative memories.
	outputHead int32
	joinKey    betaJoinKey
}
...
func (m *betaSideMemory) insert(token tokenRef, joinKey betaJoinKey) (int32, bool) {
	if m == nil || token.isZero() {
		return 0, false
	}
	ref, ok := m.indexes.insert(betaTokenRow{token: token, joinKey: joinKey})
	if !ok {
		return 0, false
	}
...
func (m *reteGraphBetaMemory) insertGeneratedAlphaOps(nodeID reteGraphAlphaNodeID, node *reteGraphAlphaNode, match conditionMatch, span *propagationCounterSpan, delta *reteAgendaDelta) (bool, error) {
	...
	m.recordAlphaFact(nodeID, match.fact)
	stage := reteGraphStageRef{kind: reteGraphStageAlpha, id: int(nodeID)}
	for _, op := range node.generatedOps {
		...
		case reteGraphGeneratedAlphaOpTerminal:
			...
			token := m.newAlphaTokenRef(stage, op.entry, match, captures, span)
			if token.isZero() {
				delta.supported = false
				continue
			}
			_, _ = m.insertTerminalToken(op.terminalID, op.branchID, token, delta, span)
...
func (m *reteGraphBetaMemory) removeGeneratedAlphaOps(nodeID reteGraphAlphaNodeID, node *reteGraphAlphaNode, match conditionMatch, counters *propagationCounterLedger, delta *reteAgendaDelta) {
	...
	for _, op := range node.generatedOps {
		...
		case reteGraphGeneratedAlphaOpTerminal:
			...
			token := m.newAlphaTokenRef(stage, op.entry, match, captures, nil)
			if token.isZero() {
				delta.supported = false
				continue
			}
...
func (m *reteGraphBetaMemory) propagateAlphaStage(source reteGraphStageRef, sourceEntry bindingTupleEntry, match conditionMatch, span *propagationCounterSpan, delta *reteAgendaDelta) error {
	...
	for _, terminal := range m.graph.stageTerminals(source) {
		...
		token := m.newAlphaTokenRef(source, entry, match, captures, span)
		if token.isZero() {
			delta.supported = false
			continue
		}
	...
	for _, successor := range m.graph.stageSuccessors(source) {
		...
		case reteGraphBetaInputRight:
			...
			token := m.newAlphaTokenRef(source, successor.entry, edgeMatch, captures, span)
			if token.isZero() {
				delta.supported = false
				continue
			}
...
func (m *reteGraphBetaMemory) propagateRemoveAlphaStage(source reteGraphStageRef, sourceEntry bindingTupleEntry, match conditionMatch, counters *propagationCounterLedger, delta *reteAgendaDelta) {
	...
	for _, terminal := range m.graph.stageTerminals(source) {
		...
		if token.isZero() {
			token = m.newAlphaTokenRef(source, entry, match, captures, nil)
		}
		if token.isZero() {
			delta.supported = false
			continue
	...
	for _, successor := range m.graph.stageSuccessors(source) {
		...
		case reteGraphBetaInputRight:
			...
			if token.isZero() {
				...
				token = m.newAlphaTokenRef(source, successor.entry, edgeMatch, captures, nil)
			}
			if token.isZero() || !m.removeBetaInputToken(successor.betaNodeID, successor.side, token, counters, delta) {
				delta.supported = false
			}
...
func (m *reteGraphBetaMemory) newAlphaTokenRef(stage reteGraphStageRef, entry bindingTupleEntry, match conditionMatch, captures []listPatternCapture, span *propagationCounterSpan) tokenRef {
	if m == nil {
		return tokenRef{}
	}
	if len(captures) == 0 {
		return m.newAlphaSourceTokenRef(stage, entry, match, span)
	}
	token := m.newTokenRef(stage, tokenRef{}, entry, conditionMatchForEntry(match, entry), match.fact.Recency(), match.fact.Generation(), span)
	if token.isZero() {
		return tokenRef{}
	}
	for _, capture := range captures {
		...
		token = m.newTokenRef(stage, token, captureEntry, captureMatch, 0, match.fact.Generation(), span)
		if token.isZero() {
			return tokenRef{}
		}
	}
...
func (m *reteGraphBetaMemory) newAlphaSourceTokenRef(stage reteGraphStageRef, entry bindingTupleEntry, match conditionMatch, span *propagationCounterSpan) tokenRef {
	if m == nil {
		return tokenRef{}
	}
	if span != nil {
	...
	previousStage := m.arena.setAllocStage(stage)
	token := m.arena.addAlphaSource(entry, match, match.fact.Recency(), match.fact.Generation())
	m.arena.allocStage = previousStage
	return token
}

func (m *reteGraphBetaMemory) newAlphaTokenRefWithRetainedCaptures(stage reteGraphStageRef, entry bindingTupleEntry, match conditionMatch, retained tokenRef, span *propagationCounterSpan) tokenRef {
	if m == nil {
		return tokenRef{}
	}
	token := m.newTokenRef(stage, tokenRef{}, entry, conditionMatchForEntry(match, entry), match.fact.Recency(), match.fact.Generation(), span)
	...
	appendRetained := func(ref tokenRef) bool {
		...
		token = m.newTokenRowRefSourceAtStage(stage, token, ref, row, 0, match.fact.Generation(), span)
		return !token.isZero()
	}
	if len(overflow) != 0 {
...
func (m *reteGraphBetaMemory) refreshAggregateOutputInternalForce(id reteGraphAggregateNodeID, bucket *reteGraphAggregateBucket, span *propagationCounterSpan, counters *propagationCounterLedger, delta *reteAgendaDelta, force bool) {
	...
	for i, value := range values {
		...
		token = m.newTokenRef(stage, token, entry, match, 0, generation, span)
		if token.isZero() {
			delta.supported = false
			return
		}
...
func accumulatorCollectValue(entries []reteGraphAggregateCollectEntry) (Value, bool) {
	...
	collected := make([]Value, len(entries))
	for i, entry := range entries {
		collected[i] = cloneValue(entry.value)
	}
...
func (m *reteGraphBetaMemory) insertBetaInput(nodeID reteGraphBetaNodeID, side reteGraphBetaInputSide, token tokenRef, entry bindingTupleEntry, span *propagationCounterSpan, delta *reteAgendaDelta) (bool, error) {
	...
	case reteGraphBetaInputLeft:
		...
		insertedRef, inserted = nodeMemory.left.insert(token, joinKey)
	case reteGraphBetaInputRight:
		var err error
		...
		insertedRef, inserted = nodeMemory.right.insert(token, joinKey)
	default:
		return false, nil
	}
	if !inserted {
	...
	case reteGraphBetaInputLeft:
		...
		matched := false
		var joinErr error
		nodeMemory.right.forEachJoinRow(joinKey, func(rightID graphTokenRowID, rightRow *betaTokenRow) bool {
			if span != nil {
			...
			output := m.appendTokenRows(reteGraphStageRef{kind: reteGraphStageBeta, id: int(nodeID)}, token, rightRow.token, span)
			if output.isZero() {
				return true
			}
			nodeMemory.outputs.link(&nodeMemory.left.indexes, insertedRef, &nodeMemory.right.indexes, int32(rightID)+1, output)
			if span != nil {
	...
	case reteGraphBetaInputRight:
		...
		matched := false
		var joinErr error
		nodeMemory.left.forEachJoinRow(joinKey, func(leftID graphTokenRowID, leftRow *betaTokenRow) bool {
			if span != nil {
				span.recordBetaCandidateRowScanned()
			}
			...
			output := m.appendTokenRows(reteGraphStageRef{kind: reteGraphStageBeta, id: int(nodeID)}, leftRow.token, token, span)
			if output.isZero() {
				return true
			}
			nodeMemory.outputs.link(&nodeMemory.left.indexes, int32(leftID)+1, &nodeMemory.right.indexes, insertedRef, output)
			if span != nil {
				span.recordBetaJoinedTokenProduced()
			}
			if err := m.propagateFromBetaNode(node, output, span, delta); err != nil {
...
func (m *reteGraphBetaMemory) insertFilterBetaInput(nodeID reteGraphBetaNodeID, side reteGraphBetaInputSide, node *reteGraphBetaNode, token tokenRef, span *propagationCounterSpan, delta *reteAgendaDelta) (bool, error) {
	...
	nodeMemory := m.nodeMemory(nodeID)
	_, inserted := nodeMemory.left.insert(token, betaJoinKey{})
	if !inserted {
		return true, nil
	}
	if span != nil {
...
// propagateStoredJoinedRemovals drains the removed row's stored output links:
// each stored output token is propagated for removal downstream exactly as
// inserted, with no key or token reconstruction and no residual
// re-evaluation, and a surviving opposite row whose link chain empties has
// lost its last match, which drives backchain demand.
func (m *reteGraphBetaMemory) propagateStoredJoinedRemovals(nodeID reteGraphBetaNodeID, side reteGraphBetaInputSide, node *reteGraphBetaNode, nodeMemory *reteGraphBetaNodeMemory, removedRow betaTokenRow, counters *propagationCounterLedger, delta *reteAgendaDelta) {
	if m == nil || delta == nil || node == nil || nodeMemory == nil {
		return
	}
	source := reteGraphStageRef{kind: reteGraphStageBeta, id: int(nodeID)}
	checkDemand := len(node.backchainDemands) > 0
	switch side {
	case reteGraphBetaInputLeft:
		nodeMemory.outputs.removeAllForLeft(removedRow.outputHead, &nodeMemory.right.indexes, func(output tokenRef, rightRef int32, rightEmpty bool) {
			m.propagateRemoveFromStage(source, output, counters, delta)
			if checkDemand && rightEmpty {
				m.appendBackchainDemandRequests(node.id, node, reteGraphBetaInputLeft, nodeMemory.right.indexes.rows[rightRef-1].token, delta)
			}
		})
	case reteGraphBetaInputRight:
		nodeMemory.outputs.removeAllForRight(removedRow.outputHead, &nodeMemory.left.indexes, func(output tokenRef, leftRef int32, leftEmpty bool) {
			m.propagateRemoveFromStage(source, output, counters, delta)
			if checkDemand && leftEmpty {
				m.appendBackchainDemandRequests(node.id, node, reteGraphBetaInputRight, nodeMemory.left.indexes.rows[leftRef-1].token, delta)
			}
		})
	default:
		delta.supported = false
...
func (m *reteGraphBetaMemory) removeRuleTerminalToken(terminalID reteGraphTerminalNodeID, branchID int, token tokenRef, counters *propagationCounterLedger, delta *reteAgendaDelta) {
	...
	m.appendRemovedTerminalDelta(delta, removed)
	if m.initialAgenda != nil && m.initialAgendaErr == nil {
		err := m.initialAgenda.applyTerminalTokenDeltasWithoutChanges(m.context(), m.revision, []reteTerminalTokenDelta{removed}, nil)
		if err != nil {
			m.initialAgendaErr = err
			delta.supported = false
		}
...
func (m *reteGraphBetaMemory) newTokenRef(stage reteGraphStageRef, parent tokenRef, entry bindingTupleEntry, match conditionMatch, recency Recency, generation Generation, span *propagationCounterSpan) tokenRef {
	if m == nil {
		return tokenRef{}
	}
	if span != nil {
	...
	previousStage := m.arena.setAllocStage(stage)
	token := m.arena.add(parent, entry, match, recency, generation)
	m.arena.allocStage = previousStage
	return token
}

func (m *reteGraphBetaMemory) newNegativeOutputTokenRef(stage reteGraphStageRef, parent tokenRef, span *propagationCounterSpan) tokenRef {
	...
	previousStage := m.arena.setAllocStage(stage)
	token := m.arena.addCompact(parent, tokenRowEntry{bindingSlot: -1}, conditionMatch{bindingSlot: -1}, 0, parent.generation())
	m.arena.allocStage = previousStage
	return token
}

func (m *reteGraphBetaMemory) newTokenRowRef(stage reteGraphStageRef, parent tokenRef, source tokenRef, row *tokenRow, match conditionMatch, recency Recency, generation Generation, span *propagationCounterSpan) tokenRef {
	if m == nil || row == nil {
		return tokenRef{}
	}
	if span != nil {
	...
	previousStage := m.arena.setAllocStage(stage)
	token := m.arena.addCompact(parent, row.tokenRowEntry(), match, recency, generation)
	m.arena.allocStage = previousStage
	return token
}
...
func (m *reteGraphBetaMemory) newTokenRowRefSource(parent tokenRef, source tokenRef, row *tokenRow, recency Recency, generation Generation, span *propagationCounterSpan) tokenRef {
	return m.newTokenRowRefSourceAtStage(reteGraphStageRef{}, parent, source, row, recency, generation, span)
}

func (m *reteGraphBetaMemory) newTokenRowRefSourceAtStage(stage reteGraphStageRef, parent tokenRef, source tokenRef, row *tokenRow, recency Recency, generation Generation, span *propagationCounterSpan) tokenRef {
	if m == nil || row == nil {
		return tokenRef{}
	}
	if span != nil {
	...
	previousStage := m.arena.setAllocStage(stage)
	token := m.arena.addCompactSource(parent, source, row.tokenRowEntry(), recency, generation)
	m.arena.allocStage = previousStage
	return token
}
...
func (m *reteGraphBetaMemory) newRootTokenRef(generation Generation, span *propagationCounterSpan) tokenRef {
	...
	previousStage := m.arena.setAllocStage(reteGraphStageRef{kind: reteGraphStageRoot})
	token := m.arena.addSeed(generation)
	m.arena.allocStage = previousStage
	return token
}
...
func (m *reteGraphBetaMemory) appendTokenRows(stage reteGraphStageRef, parent tokenRef, token tokenRef, span *propagationCounterSpan) tokenRef {
	if m == nil || token.isZero() {
		return parent
	}
	row, ok := token.resolve()
	...
	parent = m.appendTokenRows(stage, parent, token.parent(), span)
	...
	return m.newTokenRowRefSourceAtStage(stage, parent, token, row, recency, token.generation(), span)
}
...
func (m *reteGraphBetaMemory) match(ctx context.Context, source factSource) ([]ruleMatchResult, error) {
	if m == nil || m.revision == nil || m.graph == nil || source == nil {
	...
	return agendaRuleMatchResults(ctx, m.revision, agenda)
}
...
func (m *reteGraphBetaMemory) boundarySourceFacts(source factSource) []FactSnapshot {
	if m == nil || m.graph == nil || source == nil {
		return nil
	}
	var facts []FactSnapshot
	seen := make(map[FactID]struct{})
	for _, node := range m.graph.alphaNodes {
		nodeFacts, ok := source.factsForTarget(node.target)
		if !ok {
			continue
		}
		for _, fact := range nodeFacts {
			id := fact.ID()
			if !id.IsZero() {
				if _, ok := seen[id]; ok {
					continue
...
func agendaRuleMatchResults(ctx context.Context, revision *Ruleset, agenda *agenda) ([]ruleMatchResult, error) {
	if revision == nil || agenda == nil {
		return nil, ErrInvalidRuleset
	}
	byRevision := make(map[RuleRevisionID][]matchCandidate, len(revision.rulesByRevisionID))
	var firstErr error
	agenda.forEachPendingActivation(func(act *activation) bool {
		if ctx != nil {
			if err := ctx.Err(); err != nil {
				firstErr = err
				return false
		...
		if act == nil || act.token.isZero() {
			return true
		}
		rule, ok := revision.rulesByRevisionID[act.ruleRevisionID]
		if !ok {
			firstErr = fmt.Errorf("%w: unknown rule revision %q", ErrMatcher, act.ruleRevisionID)
			return false
		}
		candidate, err := buildMatchCandidateFromTokenRefWithScratch(rule, tokenRefGeneration(act.token), act.token, nil)
		if err != nil {
			firstErr = err
			return false
		}
		byRevision[rule.revisionID] = append(byRevision[rule.revisionID], candidate)
		return true
	})
	if firstErr != nil {
		return nil, firstErr
	}
	results := make([]ruleMatchResult, 0, len(revision.ruleOrder))
	for _, ruleName := range revision.ruleOrder {
		rule, ok := revision.rules[ruleName]
		if !ok {
			return nil, ErrMatcher
		}
		candidates := byRevision[rule.revisionID]
		sortMatchCandidates(revision, candidates)

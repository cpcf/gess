...
func factWorkspaceMemoryOwnerDiagnostics(workspace any) RuntimeMemoryOwnerDiagnostics {
	...
	compactFacts := value.FieldByName("compactFacts")
	if compactFacts.IsValid() && compactFacts.Kind() == reflect.Struct {
		if ids := compactFacts.FieldByName("ids"); ids.IsValid() && ids.Kind() == reflect.Slice {
			out.Rows += uint64(ids.Len())
		}
		for _, name := range []string{"ids", "templateIDs", "versions", "recencies", "supportStates", "compactSlots", "flags"} {
			slice := compactFacts.FieldByName(name)
...
func (m *reteGraphBetaMemory) betaMemoryOwnerDiagnostics() RuntimeMemoryOwnerDiagnostics {
	...
	for _, node := range m.nodes {
		...
		addBetaSideMemoryOwnerDiagnostics(&out, node.negative.right)
	}
	if out.Rows == 0 && out.Buckets == 0 && out.Indexes == 0 && out.Bytes == 0 && out.HighWater == 0 {
		return RuntimeMemoryOwnerDiagnostics{}
	}
...
func negativeBetaLeftBucketTableBytes(table negativeBetaLeftBucketTable) uint64 {
	var bytes uint64
	bytes += sliceBytes[int32](cap(table.heads))
	bytes += sliceBytes[int32](cap(table.tails))
	bytes += sliceBytes[int](cap(table.touched))
	bytes += sliceBytes[negativeBetaLeftRow](cap(table.rows))
	bytes += sliceBytes[int32](cap(table.next))
	bytes += sliceBytes[int32](cap(table.prev))
	return bytes
}

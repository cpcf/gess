...
| Goal | Start here |
| --- | --- |
| Learn the preferred `.gess` workflow | `gess-files/order_routing` |
...
| Gate on `exists` and `forall` | `higher-order/readiness_gate` |
...
### `forward-chaining/order_routing`
...
### `logical-support/remediation_tickets`
...
- `incident_response`: direct and transitive reachability proofs, and the
  clearest introduction to the mechanism.
- `insurance_claims`: proving claims payable through delegated approval
  chains, coverage, and exclusions.
- `supply_chain_impact`: transitive vulnerable-dependency impact with
  waivers.

### `modules-focus/intake_pipeline`
...
add `--serve :8080` for the browser interface. Its README maps the source
files.

### `stress`

Synthetic scaling harnesses, not tutorials: `complex_scale` generates
rulesets of fixed shapes and reports timings and memory statistics;
`extreme_scale` pushes million-scale rule and fact counts and can compare
against Jess given a `jess.jar`. Use these for local scaling experiments
only.

## The tutorial workshop

`tutorial/vulnerability_response` is an exercise: `rules.gess` starts
empty, and the reference answer lives in `solution/rules.gess`. Check
progress with:

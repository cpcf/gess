...
func (m *tokenHashMemory) reserveIndexes(joinCapacity, identityCapacity, factCapacity int) {
	...
	if joinCapacity > 0 {
		m.indexes.reserve(joinCapacity)
...
func (m *tokenHashMemory) removeRow(rowID graphTokenRowID, counters *propagationCounterLedger) {
	...
	if bucket, ok := m.indexes.get(removed.joinKey); ok {
		if bucket.remove(rowID) {
			if bucket.len() == 0 {
				m.recycleBucketRest(bucket.rest)
				m.indexes.delete(removed.joinKey)
			} else {
				m.indexes.set(removed.joinKey, bucket)
			}
		}
	}
	...
	if index != last {
		...
		if bucket, ok := m.indexes.get(moved.joinKey); ok {
...
func (m *reteGraphBetaMemory) recordGeneratedTerminalRow(factID FactID, nodeID reteGraphAlphaNodeID, terminalID reteGraphTerminalNodeID, branchID int, handle graphTokenRowHandle) {
	...
	m.alphaFactTerminalRows[factID] = rows
}
...
func (m *reteGraphBetaMemory) takeGeneratedTerminalRow(factID FactID, nodeID reteGraphAlphaNodeID, terminalID reteGraphTerminalNodeID, branchID int) (graphTokenRowHandle, bool) {
	if m == nil || factID.IsZero() || m.alphaFactTerminalRows == nil {
		return graphTokenRowHandle{}, false
	}
	rows := m.alphaFactTerminalRows[factID]
	for i, row := range rows {
		...
		rows = rows[:last]
		if len(rows) == 0 {
			delete(m.alphaFactTerminalRows, factID)
		} else {
			m.alphaFactTerminalRows[factID] = rows
		}
		return handle, true
...
func (m *reteGraphBetaMemory) materializeQueryTerminalRows(ctx context.Context, query compiledQuery, args map[string]Value, source Snapshot, terminalIDs []reteGraphTerminalNodeID, triggerID FactID) ([]QueryRow, error) {
	...
	for _, terminalID := range terminalIDs {
		...
		for _, terminalRow := range terminal.rows.rows {
...
func (m tokenHashMemory) diagnostics() reteGraphTokenMemoryDiagnostics {
	...
	m.indexes.forEachBucket(func(bucket graphTokenRowIDBucket) {
		depth := bucket.len()
